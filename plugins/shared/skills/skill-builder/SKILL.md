---
name: "Skill Builder"
description: "Create new Claude Code Skills with proper YAML frontmatter, progressive disclosure structure, and complete directory organization. Use when building custom skills, creating skill templates, designing skill architecture, or understanding the skill specification. Triggers: skill creation, custom skill, skill design, skill building, skill generator, build skill, skill development, skill pattern, Claude Code skill"
---
...
Claude should automatically activate **skill-builder** when:
- ✅ User mentions "create a new skill", "build a custom skill", or "skill generator"
- ✅ User asks "How do I create a Claude Code skill?" or "What's the skill format?"
- ✅ User needs guidance on YAML frontmatter, skill structure, or skill design
- ✅ User requests skill templates: "skill template", "skill example", or "skill pattern"
- ✅ User wants to understand skill specification or progressive disclosure
- ✅ User is working on skill organization: "skill directory", "skill file structure"

Claude should **NOT** activate this skill for:
- ❌ General programming questions (use coder/reviewer skills)
- ❌ Creating regular code files that aren't Claude Code skills
- ❌ Troubleshooting existing application code (Python/JavaScript/etc.)
- ❌ Documentation writing that isn't skill-related (use documenter skill)
...
### Q: How do I know if my skill will activate properly?

**A**: Test your trigger keywords:
1. Create a test message mentioning your trigger keywords
2. If Claude mentions the skill or offers to use it, activation is working
3. If nothing happens, your trigger keywords might be too generic or conflicting with other skills
4. Check against existing skills to ensure no keyword overlap

**Example test**:
```
You: "I need to create a custom skill for my workflow"
Claude should: Mention skill-builder or offer to help with skill creation
```
...
| Aspect | Personal Skills | Project Skills |
|--------|---|---|
| **Location** | `~/.claude/skills/` | `.claude/skills/` (in project root) |
| **Scope** | Available in ALL projects | Available ONLY in this project |
| **Version Control** | NOT committed to git | SHOULD be committed to git |
| **Use Case** | Personal productivity tools | Team workflows, shared knowledge |

Choose project skills for team-shared workflows so they're versioned and consistent.

---

### Q: Can I have multiple trigger keywords?

**A**: Yes, and you should! Optimal range: **5-10 keywords**

**Too few** (1-2): Skill rarely activates when needed
```yaml
# ❌ Bad: Won't catch variations
Triggers: skill
```

**Just right** (5-10): Catches most variations without being too generic
```yaml
# ✅ Good: Covers variations
Triggers: skill creation, custom skill, skill design, skill building, skill generator, build skill
```

**Too many** (20+): Becomes generic, activates on unrelated topics
```yaml
# ❌ Bad: Too generic
Triggers: code, help, build, create, design, write, make, generate, development, project...
```

---

### Q: Should I keep SKILL.md small or comprehensive?

**A**: Use **progressive disclosure** architecture:

| Size | Level | Content |
|------|-------|---------|
| **2-5 KB** | Core (SKILL.md) | What skill does, quick start, common scenarios |
| **5-10 KB** | Extended | Advanced features, detailed guides, scripts |
| **10+ KB** | Move to separate files | Put in docs/, examples/, templates/ |

**Example organization**:
```
.claude/skills/my-skill/
├── SKILL.md (2-5 KB) ← Main instructions
├── docs/
│   ├── ADVANCED.md (5 KB) ← Detailed reference
│   └── TROUBLESHOOTING.md (3 KB) ← Problem solutions
├── resources/
...
Claude loads only SKILL.md initially. Other files load on-demand.

---

### Q: What if my skill name conflicts with an existing one?
...
# Choose unique, descriptive name instead
```

**Examples**:
```
❌ "Skill Builder" (maybe too generic)
✅ "Python Testing Framework" (specific)
✅ "React Component Generator" (specific)
```

---

### Q: Can I update a skill after deployment?

**A**: Yes! Skills are living documents:

1. Update SKILL.md content
2. Keep `name` and core trigger keywords stable
3. Add new trigger keywords if needed
4. Increment version number (v1.0 → v1.1 → v2.0)
5. Document changes in footer or version history

**Version scheme**:
- **v1.0 → v1.1**: Minor improvements (new example, better explanation)
- **v1.0 → v2.0**: Major changes (restructured workflow, breaking changes)

---

### Q: How do I know if my YAML is valid?

**A**: Validate with Python:
...
**Common errors**:
- Missing quotes around strings with colons: `name: My:Skill` → `name: "My:Skill"`
- Inconsistent indentation (use 2 spaces)
- Missing `---` delimiters

---

### Q: Should I use `allowed-tools` in my skill?

**A**: Only if needed. Recommended values:

| Skill Type | Tools | Reason |
|-----------|-------|--------|
| Documentation | `Read, Write` | Create/edit docs |
| Code generation | `Read, Write, Edit, Bash` | Create and test code |
| Analysis | `Read, Glob, Grep` | Don't modify files |
| Build/Deploy | `Read, Bash` | Run build commands |

Don't include tools you won't use. Security principle: minimal permissions.

---

### Q: How long should I spend on a skill?

**A**: Time breakdown by complexity:

| Skill Complexity | Time Investment | Content | Examples |
|---|---|---|---|
| **Simple** | 2-4 hours | 1000-2000 words | 2-3 templates |
| **Medium** | 4-8 hours | 2000-5000 words | 3-5 examples, scripts |
| **Complex** | 8-16 hours | 5000+ words | 5+ examples, multiple docs |

Focus on quality over quantity. A 2000-word skill that's useful beats a 10000-word skill that's confusing.

---

### Q: Can I make a skill that creates other skills?

**A**: Yes! Meta-skills are powerful but complex:

1. Have core skill document the process
2. Use templates for generated skills
3. Include validation for generated output
4. Document patterns in skill-creator documentation

Example: skill-creator creates other skills by following the framework and templates.
...
**Maintained By**: agentic-flow team / neural-trader project
**License**: MIT

## Version History

### v1.1.0 (2025-10-24)
- Enhanced with trigger keywords, "When to Use" section, comprehensive Q&A troubleshooting, and improved YAML frontmatter for better Claude Code skill activation

### v1.0.0 (2025-10-19)
- Initial creation with complete skill building templates, YAML frontmatter specification, progressive disclosure architecture, and comprehensive Q&A troubleshooting for creating production-ready Claude Code Skills
